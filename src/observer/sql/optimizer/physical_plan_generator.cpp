...
RC PhysicalPlanGenerator::create_plan(AggreLogicalOperator &aggr_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  if (!child_opers.empty()) {
    ...
    rc = create(*child_oper, child_phy_oper);
    if (rc != RC::SUCCESS) {
      LOG_WARN("failed to create aggre logical operator's child physical operator. rc=%s", strrc(rc));
      return rc;
    }
  }

  unique_ptr<AggrePhysicalOperator> aggre_operator;
  if(aggr_oper.having_flag()){
...
RC PhysicalPlanGenerator::create_plan(InsertLogicalOperator &insert_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  vector<ValueRecord> &valuerecords = insert_oper.valuerecords();
  InsertPhysicalOperator *insert_phy_oper = new InsertPhysicalOperator(table, std::move(valuerecords));
...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_oper, std::unique_ptr<PhysicalOperator> &oper)
{
  ...
  vector<std::string> value_name = update_oper.value_name();
  ...
  oper = unique_ptr<PhysicalOperator>(new UpdatePhysicalOperator(update_oper.table(), values, value_name));
...
RC PhysicalPlanGenerator::create_plan(OrderLogicalOperator &order_oper, std::unique_ptr<PhysicalOperator> &oper)
{
  ...
  OrderPhysicalOperator *order_operator = new OrderPhysicalOperator(order_oper.orderby_stmt());
