...
RC OrderByStmt::create(Db *db, Table *default_table, std::unordered_map<std::string, Table *> *tables,std::unordered_map<std::string, Table *> *alias_tables,
     std::vector<OrderBySqlNode> &orderbys, int orderby_num, OrderByStmt *&stmt)
{
  RC rc = RC::SUCCESS;
  stmt = nullptr;
  ...
  for (int i = 0; i < orderby_num; i++) {
    OrderByUnit *orderby_unit = nullptr;
    rc = create_orderby_unit(db, default_table, tables,alias_tables, orderbys[i], orderby_unit);
    if (rc != RC::SUCCESS) {
      delete tmp_stmt;
      LOG_WARN("failed to create filter unit. condition index=%d", i);
      return rc;
...
RC OrderByStmt::create_orderby_unit(Db *db, Table *default_table, std::unordered_map<std::string, Table *> *tables,std::unordered_map<std::string, Table *> *alias_tables,
     OrderBySqlNode &orderby, OrderByUnit *&orderby_unit)
{
  RC rc = RC::SUCCESS;
  ...
  relAttr->attribute_name = orderby.field;
  relAttr->relation_name = orderby.order_relation; 
  rc = get_table_and_field(db, default_table, tables,alias_tables, *relAttr, table, field);
  if (rc != RC::SUCCESS) {
    LOG_WARN("cannot find attr");
    return rc;
  }
