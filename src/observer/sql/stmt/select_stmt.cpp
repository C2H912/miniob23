...
void dfs(Expression* current, std::vector<RelAttrSqlNode>& attr, std::vector<std::string>& relation)
{
  if(current->type() == ExprType::VALUE){
    ...
    RelAttrSqlNode node;
    node.aggr_func = fieldnode->aggre_name();
    node.attribute_name = fieldnode->str_attribute_name();
    node.relation_name = fieldnode->str_table_name();
    attr.push_back(node);
    relation.push_back(fieldnode->str_table_name());
    return;
  }
  ArithmeticExpr* arithnode = static_cast<ArithmeticExpr*>(current);
  if(arithnode->left() != nullptr){
    dfs(arithnode->left(), attr, relation);
  }
  if(arithnode->right() != nullptr){
    dfs(arithnode->right(), attr, relation);
  }
}
...
// ---------- 首先通过深度优先搜索把expression树中的内容解析出来 ----------
  std::vector<RelAttrSqlNode> all_relAttrNodes;
  std::vector<std::string> all_relations;
  for(size_t i = 0; i < select_sql.expressions.size(); i++){
    dfs(select_sql.expressions[i], all_relAttrNodes, all_relations);
  }
